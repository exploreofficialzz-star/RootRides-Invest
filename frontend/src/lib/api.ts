...
export interface Plan {
  id: string; name: string; amount_naira: number;
  daily_return_naira: number; image_path: string;
  gradient: string; is_dark: boolean; grid_span: string; display_order: number;
}
export interface PlatformStats {
  total_invested: number; active_users: number;
  uptime_percent: number; support_label: string;
}
export interface Testimonial {
  id: string; quote: string; name: string;
  plan_label: string; display_order: number;
}
export interface FaqItem {
  id: string; question: string; answer: string; display_order: number;
}
...
export interface UserStats {
  total_invested: number; total_earned: number;
  today_potential: number; has_investments: boolean;
  referral_code: string | null; referral_earnings: number;
}
export interface Investment {
  id: string; user_id: string; plan_id: string; plan_name: string;
  amount_naira: number; daily_return_naira: number;
  is_active: boolean; started_at: string;
}
...
// Authenticated requests — sends Bearer token
...
export const api = {
  ...
  // Auth
  register: (phone: string, full_name: string, password: string) => post<AuthResponse>("/api/auth/register", { phone, full_name, password }),
  ...
  createInvestment:  (plan_id: string, plan_name: string, amount_naira: number, daily_return_naira: number) =>
    authPost("/api/investments", { plan_id, plan_name, amount_naira, daily_return_naira }),
